...
void *communicate(void *message)
{
	...
	//--------------------------------------------------------------------------//
	// Take name and VOTER_ID from client

	//Receive Name/VOTER_ID from the client
	...
	// Read Voters_List.txt and match VOTER_ID and name, if match_exists then allow to vote else end thread
	FILE *fp;
	char buff[255];
